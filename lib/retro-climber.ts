...
class SoundSystem {
  ...
  public playCrumble() {
    ...
    osc.type = 'square';
    // Noise-like rapid vibrato / pitch drop
...
export class GameEngine {
  ...
  private resetGame() {
    ...
    this.platforms.push({
      ...
      x: 0,
      y: groundHeight, // Ground top sits at dynamic Y
  ...
  private drawPlatform(platform: Platform) {
    ...
    switch (platform.type) {
      ...
      case 'MOVING':
        // Steel platform with industrial metallic textures
        this.ctx.fillStyle = '#4b6584'; // metallic slate blue
        this.ctx.fillRect(x, screenY, platform.width, platform.height);

        // Dark industrial shading
        this.ctx.fillStyle = '#2f3542';
        this.ctx.fillRect(x, screenY + platform.height - 3, platform.width, 3);
        this.ctx.fillRect(x, screenY, 3, platform.height);
        this.ctx.fillRect(x + platform.width - 3, screenY, 3, platform.height);

        // Light blue neon top edge strip
        this.ctx.fillStyle = '#90caf9';
        this.ctx.fillRect(x + 2, screenY, platform.width - 4, 3);

        // Rivets/Bolts on corners
        this.ctx.fillStyle = '#dcdde1'; // metal white
        this.ctx.fillRect(x + 5, screenY + 7, 3, 3);
        this.ctx.fillRect(x + platform.width - 8, screenY + 7, 3, 3);

        // Pulsing neon indicator light in the center
        const pulse = Math.floor(Date.now() / 250) % 2;
        this.ctx.fillStyle = pulse === 0 ? '#38bdf8' : '#f43f5e'; // alternating cyan and pink
        this.ctx.fillRect(x + platform.width / 2 - 4, screenY + 5, 8, 4);
        break;

      case 'CRUMBLING':
        // Shaking, cracked sandstone platform
        this.ctx.fillStyle = '#d2b48c'; // tan/sandstone
        this.ctx.fillRect(x, screenY, platform.width, platform.height);
        ...
        // Crack lines
        this.ctx.fillStyle = '#4e342e'; // dark brown cracks
        this.ctx.fillRect(x + 12, screenY, 2, 8);
        this.ctx.fillRect(x + 12, screenY + 8, 8, 2);
        this.ctx.fillRect(x + 20, screenY + 8, 2, 10);

        this.ctx.fillRect(x + platform.width - 24, screenY, 2, 6);
        this.ctx.fillRect(x + platform.width - 26, screenY + 6, 6, 2);
        this.ctx.fillRect(x + platform.width - 26, screenY + 8, 2, 10);

        // If currently crumbling, draw falling pixels
        if (platform.crumbleTimer !== undefined && platform.crumbleTimer < 20) {
          this.ctx.fillStyle = '#8d6e63';
          this.ctx.fillRect(x + Math.sin(platform.crumbleTimer) * 10 + platform.width/2, screenY + 12, 3, 3);
          this.ctx.fillRect(x + Math.cos(platform.crumbleTimer) * 15 + platform.width/3, screenY + 14, 2, 2);
        }
        break;

      case 'SPRING':
        // 1. Wooden plank base
        this.ctx.fillStyle = '#8d6e63'; // wood brown
        this.ctx.fillRect(x, screenY + 6, platform.width, platform.height - 6);
        this.ctx.fillStyle = '#5d4037'; // dark wood borders
        this.ctx.fillRect(x, screenY + platform.height - 3, platform.width, 3);

        // Plank dividers
        for (let px_idx = x + 16; px_idx < x + platform.width; px_idx += 20) {
          this.ctx.fillRect(px_idx, screenY + 6, 2, platform.height - 6);
        }

        // 2. Metal spring coiled on top
        const springX = x + platform.width / 2 - 12;
        const springY = screenY - 4; // sits slightly on top
        this.ctx.fillStyle = '#b2bec3'; // silver steel coil

        if (platform.springState === 'compressed') {
          // Flattened squished spring
          this.ctx.fillRect(springX, springY + 6, 24, 4);
          this.ctx.fillStyle = '#d63031'; // glowing red spring base
          this.ctx.fillRect(springX + 4, springY + 3, 16, 3);
        } else if (platform.springState === 'extended') {
          // Tall stretched spring
          this.ctx.fillRect(springX + 8, springY - 14, 8, 4); // cap
          this.ctx.fillRect(springX + 4, springY - 10, 16, 4); // upper loop
          this.ctx.fillRect(springX, springY - 4, 24, 4); // mid loop
          this.ctx.fillRect(springX + 4, springY + 2, 16, 4); // lower loop
        } else {
          // Idle standard state spring
          this.ctx.fillRect(springX + 6, springY - 4, 12, 4); // upper cap
          this.ctx.fillRect(springX + 2, springY + 1, 20, 4); // lower coil
          this.ctx.fillStyle = '#e67e22'; // orange alert tip
          this.ctx.fillRect(springX + 10, springY - 7, 4, 3);
        }
        break;
    }
